    ...
    <footer className="bg-gray-900 text-white">
      <div className="container-custom py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div>
            <h3 className="font-semibold text-lg mb-4">{packages.title}</h3>
            <ul className="space-y-2 text-gray-400">
              {packages.links.map((link: FooterLink) => (
                <li key={link.text}><Link to={link.url} className="hover:text-primary-300">{link.text}</Link></li>
              ))}
            </ul>
          </div>
          
          <div>
            <h3 className="font-semibold text-lg mb-4">{company.title}</h3>
            <ul className="space-y-2 text-gray-400">
              {company.links.map((link: FooterLink) => (
                <li key={link.text}><Link to={link.url} className="hover:text-primary-300">{link.text}</Link></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            <h3 className="font-semibold text-lg mb-4">{customerCenter.title}</h3>
            <ul className="space-y-2 text-gray-400">
              {customerCenter.info.map((item: string) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
            ...
            <div className="mt-4 flex space-x-4">
              {social.map((item: SocialLink) => (
                <a key={item.name} href={item.url} className="text-gray-400 hover:text-white">
